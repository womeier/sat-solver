...
def build(theme_name):
    ...
    a('<desc id="figDesc">Mean solve time per instance on a logarithmic scale, for three solvers across three '
      ...
      'and 0.51 ms worst; naive 192 ms mean and 560 ms worst; naive functional 480 ms mean and 863 ms worst. '
      'On uf50-218 (50 variables, satisfiable): dpll 5.70 ms mean, 20.4 ms worst. On uuf50-218 (50 variables, '
      'unsatisfiable): dpll 15.8 ms mean, 58.9 ms worst. Both naive solvers are out of reach on the '
      '50-variable sets, since they enumerate all 2^50 valuations.</desc>')
    a(f'<rect x="0" y="0" width="{W}" height="{H}" fill="{surface}"/>')

    block = HEADER_H + 3 * ROW_H + GROUP_GAP
